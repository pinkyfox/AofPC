...
void speaker(int turnOn)
{
	...
	if (turnOn)
	{	
		result |= 0x03;		// включить спикер
	}
	else
	{
		result &= 0xFC;		// выключить спикер
	}
	outp(0x61, result);
}
...
void set_frequence(unsigned frequence)
{
	...
	outp(0x43, 0xB6);	// канал 2, прямоугольные импульсы

	result = frequence_delimeter & 0x00FF;		// младший байт делителя частоты
	outp(0x42, result);

	result = frequence_delimeter >> 8;			// старший байт делителя частоты
	outp(0x42, result);
}
#undef TIMER_CLOCK
